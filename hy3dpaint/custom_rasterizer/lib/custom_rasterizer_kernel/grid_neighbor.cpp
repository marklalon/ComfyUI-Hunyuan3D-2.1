    ...
    std::vector<torch::Tensor> view_layer_normals, int num_level, int resolution)
{
    ...
    texture_positions[0] = torch::zeros({(int64_t)(seq2pos.size() / 3), (int64_t)3}, float_options);
    ...
    for (int i = 0; i < grids.size(); ++i) {
        grid_neighbors[i] = torch::zeros({(int64_t)grids[i].seq2grid.size(), (int64_t)9}, int64_options);
